 ...
 */
@AIName("star_shop")
public class StarShopAI2 extends GeneralNpcAI2 {
    ...
    public void addStarcard(final Player player) {
		...
		if (player.getInventory().isFull()) {
			...
			return;
                ...
                ItemService.addItem(player, 188710186, 1);
                DAOManager.getDAO(InGameShopLogDAO.class).log("BUY", new Timestamp(System.currentTimeMillis()), player.getName(), player.getAcountName(), player.getName(), 188710186, 1, 100);
		DAOManager.getDAO(InventoryDAO.class).store(player);
                log.info("[INGAMESHOP] > Account name: " + player.getAcountName() + ", PlayerName: " + player.getName() + " buy VIP cost 100 toll.");
        ...
        if (requested) {
            PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(902247, 0, 0, MuiService.getInstance().getMessage("COST") + toll + " Points. " + MuiService.getInstance().getMessage("ALL_POINTS") + tolls + " Points. "
                    + MuiService.getInstance().getMessage("BUY_STAR")));
        }
    }
}
