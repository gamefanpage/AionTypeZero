...
@AIName("icons_katalam")
public class IconsTheKatalamAI2 extends AggressiveNpcAI2 {
	...
	protected void handleCreatureSee(Creature creature) {
		...
		if (creature instanceof Player) {
			...
			final GaleCycloneObserver observer = new GaleCycloneObserver(player, getOwner()) {
				...
				public void onMove() {
					if (!blocked) {
						SkillEngine.getInstance().getSkill(getOwner(), 20373, 65, player).useNoAnimationSkill();
					}
				}
