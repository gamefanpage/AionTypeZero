...
@AIName("legion_gaubice_small")
public class LegionGaubiceSmallAI2 extends ActionItemNpcAI2 {
	...
	public void handleUseItemFinish(Player player) {
		  ...
		  switch (player.getRace()) {
			case ASMODIANS:
		  SkillEngine.getInstance().applyEffectDirectly(21061, player, player, 160000 * 3);
		  break;
			case ELYOS:
		  SkillEngine.getInstance().applyEffectDirectly(21060, player, player, 160000 * 3);
		  break;
		  }
		  PacketSendUtility.sendBrightYellowMessageOnCenter(player, MuiService.getInstance().getMessage("LEGION_GAUBICE_SMALL"));
		  stopMove(player);
