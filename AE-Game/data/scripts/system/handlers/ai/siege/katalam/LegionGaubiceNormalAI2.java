...
@AIName("legion_gaubice_normal")
public class LegionGaubiceNormalAI2 extends ActionItemNpcAI2 {
	...
	public void handleUseItemFinish(Player player) {
		  ...
		  switch (player.getRace()) {
			case ASMODIANS:
		  SkillEngine.getInstance().applyEffectDirectly(21386, player, player, 160000 * 3);
		  break;
			case ELYOS:
		  SkillEngine.getInstance().applyEffectDirectly(21385, player, player, 160000 * 3);
		  break;
		  }
		  PacketSendUtility.sendBrightYellowMessageOnCenter(player, MuiService.getInstance().getMessage("LEGION_GAUBICE_NORMAL"));
		  stopMove(player);
