 ...
 * @author Romanz
 */
@AIName("7011_mercenary_asmo")
public class MercenaryAsmodians_7011AI2 extends NpcAI2 {
    ...
    private void checkEntryConditions(Player player, int dialogId, int questId) {
        if (dialogId == 10000) {
            if (player.getInventory().getItemCountByItemId(186000236) >= 12) {
                if ((getPosition().getWorldMapInstance().getNpc(881597)  == null) &&
                player.getInventory().decreaseByItemId(186000236, 12)) {
                spawn(881597, 846.1f, 310.01f, 150.94f, (byte) 0); // Ra
                spawn(881597, 846.1f, 306.01f, 150.94f, (byte) 0); // Ra
                spawn(881597, 781.6f, 345.08f, 153.7f, (byte) 30); // Ra
                spawn(881597, 835.02f, 334.4f, 150.94f, (byte) 30); // Ra
                spawn(881597, 839.1f, 332.01f, 150.94f, (byte) 30); // Ra
                spawn(881597, 792.03f, 344.96f, 153.7f, (byte) 30); // Ra
                PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402485));
                PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402492));
            ...
            if (player.getInventory().getItemCountByItemId(186000236) >= 12) {
                if ((getPosition().getWorldMapInstance().getNpc(882294)  == null) &&
                player.getInventory().decreaseByItemId(186000236, 12)) {
                spawn(882294, 839.74f, 353.79f, 142.19f, (byte) 0); // trap
                spawn(882294, 863.15f, 336.27f, 141.94f, (byte) 0); // trap
                spawn(882294, 710.87f, 336.48f, 141.94f, (byte) 0); // trap
                spawn(882294, 734.72f, 353.71f, 142.19f, (byte) 35); // trap
                PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402486));
                PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402493));
                PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(), 2375));
                } else {
                PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1401837));
                PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(), 10));
    ...
    protected void handleDespawned() {
        ...
        deleteMobs(instance.getNpcs(881597));
        deleteMobs(instance.getNpcs(882294));
