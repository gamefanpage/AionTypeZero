	...
	private void checkEntryConditions(Player player, int dialogId, int questId) {
		if (dialogId == 10000) {
			  if (player.getInventory().decreaseByItemId(185000137, 1)) {
				...
				PacketSendUtility.sendBrightYellowMessageOnCenter(player, MuiService.getInstance().getMessage("TANK_BASTION"));
				AI2Actions.deleteOwner(this);
				} else {
		    PacketSendUtility.sendBrightYellowMessageOnCenter(player, MuiService.getInstance().getMessage("TANK_BASTION_NO"));
